...
describe("persistence test", () => {
    ...
    it('teste /partida/list e /partida/delete', async () => {
        var agent = supertest(app);
        const postList = await agent.get('/partidas');
        expect(postList.statusCode).toEqual(200);
        if (postList.body.length > 0){
        for(const e of postList.body){
           
            const data = { "id" : e.id };
            console.log("Encontrou o partida: ");
            console.log(data);
            
            const postDelete = await agent.delete('/partidas').send(data);
            
            console.log("Removeu o partida: ");
            console.log(data);

            expect(postDelete.statusCode).toEqual(204);
        }
        }else{
            console.log("Partidas não cadastradas")
        }
    });
